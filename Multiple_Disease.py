...
alzheimer = pickle.load(open("alzheimer_model1.sav","rb"))
...
if (selected == "Alzheimer"):
    ...
    if st.button("Show relations"):
        folder = "AlzheimerImages"
        for i in os.listdir('./AlzheimerImages'):
            st.image(os.path.join(folder,i))
        if st.button("Hide relations"):
            pass
    introduction =pd.DataFrame({
    "Title" :["Sex  (1) Male (0) Female","Age",'educ','ses','cdr','mmse','etiv','nwbv','asf'],
    "Description" : ['Sex',"Age",'Years of education','Socioeconomic status','Clinical dimentia rating','Mini mental state examination','Estimated total intracranical volumne','Normalized whole brain volume','Atlas scaling factor']
    })
    ...
    if st.button("Show statistics"):
        ...
        for i in range(len(avgAlz)):
            ...
            ax = plt.axes()
            ax.bar(x,y,label=Features[i],color=["#390099","#ff0054"])
            ax.legend()
            # st.pyplot(fig)
            buf = BytesIO()
            ...
            images.append(buf)
        try:
            for buf in range(0,len(images),2):
                st.image([images[buf],images[buf+1]])
        except:
            pass
            # if i <= 5:
            #     with col1:
...
if (selected == "Diabetes"):
    st.title("Diabetes Prediction using ML for Women")
    if st.button("Show relations"):
        folder = "DiabetesImages"
        for i in os.listdir('./DiabetesImages'):
            st.image(os.path.join(folder,i))
        if st.button("Hide relations"):
            pass
...
if (selected == "Heart Disease"):
    ...
    if st.button("Show relations"):
        folder = "HeartImages"
        for i in os.listdir('./HeartImages'):
            st.image(os.path.join(folder,i))
        if st.button("Hide relations"):
            pass
    ...
    if st.button("Show statistics"):
        ...
        for i in range(len(avgHeart)):
            ...
            plt.figure()
            fig = plt.figure(figsize=(4,5)) # try different values
            ax = plt.axes()
            ax.bar(x,y,label=Features[i],color=["#390099","#ff0054"], alpha=0.85)
            ax.legend()
            # st.pyplot(fig)
            buf = BytesIO()
            fig.savefig(buf, format="png")
